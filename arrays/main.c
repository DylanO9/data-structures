...
Vector *init_vector(size_t elem_size) {
    ...
    if (new_vector == NULL) {
        fprintf(stderr, "Could not create a new vector\n");
        return NULL;
    }
...
void append(Vector *v, void *object) {
    ...
    void *next_element = (char *)v->data + (v->size * v->elem_size); 
...
void *get(Vector *v, unsigned int index) {
    ...
    return (char *)v->data + (v->elem_size * index);
}
...
void *search(Vector *v, void *object) {
    for (int i = 0; i < v->size; i++) {
        ...
        void *iterator_address = base + (i * v->elem_size);    
        if (memcmp(iterator_address, object, v->elem_size) == 0)
